...
class ConversationController extends Controller
{
    public function get_admin_message(Request $request)
    {
        ...
        $messages = Conversation::where(['user_id' => $request->user()->id])->where(function($query) use ($request){
            if($request->has($request->branch_id)){
    ...
    public function store_admin_message(Request $request)
    {
        ...
        try {
            ...
            if($request->branch_id){
                $conv->restaurant_id = $request->branch_id;
            }
