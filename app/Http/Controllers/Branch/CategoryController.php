...
class CategoryController extends Controller
{
    function index(Request $request)
    {
        ...
        $categories = $categories->where('branch_id',auth('branch')->id())->latest()->paginate(Helpers::getPagination())->appends($query_param);
    ...
    function sub_index(Request $request)
    {
        ...
        $categories = Category::with(['parent'])
            ...
            ->where(['position' => 1,'branch_id' => auth('branch')->id()])
            ->latest()
    ...
    function store(Request $request)
    {
        ...
        if ($request->has('type')) {
            $request->validate([
                ...
                'parent_id.required' => \App\CentralLogics\translate('Select a category first')
            ]);
        }
        ...
        //uniqueness check
        $cat = Category::where('name', $request->name)->where('branch_id',auth('branch')->id())->where('parent_id', $request->parent_id ?? 0)->first();
        if (isset($cat)) {
        ...
        $category->branch_id = auth('branch')->id();
        $category->save();
    ...
    {
        $category = category::withoutGlobalScopes()->with('translations')->where('branch_id',auth('branch')->id())->find($id);
