...
class OrderController extends Controller
{
    ...
    public function details($id)
    {
        ...
        return view('admin-views.order.order-view', compact('order','nearestDeliveryMan'));
    }
    ...
    //         'status' => 'pending', // Change the status to 'accepted'
    //     ]);
    ...
    //     // Step 2: When the delivery man accepts the request
    //     // Update the status in the `delivery_requests` table
    ...
    public function add_delivery_man($order_id, $delivery_man_id)
    {
        // Check if the delivery man ID is valid
        if ($delivery_man_id == 0) {
            return response()->json([], 401);
        }
        ...
        // Check if the order is in a state where assignment is allowed
        if (
