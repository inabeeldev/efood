...
class Helpers
{
    ...
    public static function product_data_formatting($data, $multi_data = false)
    {
        ...
        if ($multi_data == true) {
            foreach ($data as $item) {
                ...
                $addOnes = [];
                foreach (AddOn::whereIn('id', json_decode($item['add_ons']))->get() as $itemAddon) {
                    $itemAddon->branch_id = $itemAddon->branch_id ?? '';
                    $addOnes[] = $itemAddon;
                }
            ...
            $addOnes = [];
            foreach (AddOn::whereIn('id', $addon_ids)->get() as $itemAddon) {
                $itemAddon->branch_id = $itemAddon->branch_id ?? '';
                $addOnes[] = $itemAddon;
            }
    ...
    public static function get_business_settings($name)
    {
        ...
        // $data = \App\Model\BusinessSetting::where(['key' => $name,'branch_id' => auth('branch')->id()])->first();
        if (isset($data)) {
...
//                $data[] = ['id'=> $addon->id, 'name'=>$addon->name, 'price'=>$addon->price, 'quantity'=> $add_on_qty];
    ...
    {
        $data = self::get_branch_business_settings('language');
        $default_lang = 'en';
