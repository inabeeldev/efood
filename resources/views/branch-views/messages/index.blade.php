                    ...
                    <div class="py-4" id="conversation_sidebar">
                        <div class="chat_people media px-3 gap-3 mb-4">
                            <div class="avatar position-relative">
                                <img src="{{asset('/storage/app/public/admin/'.auth('branch')->user()->image)}}" class="img-fit rounded-circle" onerror="this.src='{{asset('public/assets/admin')}}/img/160x160/img1.jpg'">
                                <span class="avatar-status status-sm bg-success"></span>
                            </div>
                            <div class="chat_ib media-body">
                                <h5 class="mb-0">{{auth('branch')->user()->f_name}} {{auth('branch')->user()->l_name}}</h5>
                                <span class="fz-12">{{auth('branch')->user()->phone}}</span>
                            </div>
                        </div>
...
{{--                                @dump($conv->user_id)--}}
                                ...
                                <div class="sidebar_primary_div d-flex justify-content-between align-items-center cursor-pointer customer-list {{$unchecked!=0?'conv-active':''}}"
                                    onclick="viewConvs('{{route('branch.message.view',[$conv->user_id])}}','customer-{{$conv->user_id}}')"
...
@push('script_2')
    ...
    <script>
        ...
        function update_fcm_token(token) {
            ...
            $.ajax({
                type: "POST",
                url: "{{route('branch.message.update_fcm_token')}}",
                data: {
                    fcm_token: token,
                },
                cache: false,
