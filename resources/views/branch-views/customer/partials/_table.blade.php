@foreach($customers as $key=>$customer)
    <tr class="">
        ...
        <td class="max-w300">
            <a class="text-dark media align-items-center gap-2" href="{{route('branch.customer.view',[$customer['id']])}}">
                <div class="avatar">
        ...
        <td>
            <div><a href="mailto:" class="text-dark"><strong>{{preg_replace('/(?<=.).(?=.*@)/', '*', $customer['email'])}}</strong></a></div>
            <div><a class="text-dark" href="tel:">{{preg_replace('/(?<=.).(?=.*.)/', '*', $customer['phone'])}}</a></div>
        </td>
        <td>
        ...
        <td>
            <label class="switcher">
                <input id="{{$customer['id']}}" data-url="{{route('branch.customer.update_status', ['id' => $customer['id']])}}" onclick="status_change(this)" type="checkbox" class="switcher_input" {{$customer->is_active == 1? 'checked' : ''}}>
        ...
        <td>
            <div class="d-flex justify-content-center gap-2">
                <a class="btn btn-outline-success btn-sm square-btn"
                    href="{{route('branch.customer.view',[$customer['id']])}}">
                    <i class="tio-visible"></i>
                </a>
                <a class="btn btn-outline-info btn-sm square-btn" href="javascript:" onclick="set_point_modal_data('{{route('branch.customer.set-point-modal-data',[$customer['id']])}}')">
                    <i class="tio-coin"></i>
                </a>
                ...
                <form id="customer-{{$customer['id']}}" action="{{route('branch.customer.destroy',['id' => $customer['id']])}}" method="post">
                    @csrf
                    @method('DELETE')
                </form>
            </div>
