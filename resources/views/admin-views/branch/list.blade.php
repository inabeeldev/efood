                    ...
                    <div class="col-sm-8 col-md-6 col-lg-4">
                        <form action="#" method="GET">
                            <div class="input-group">
                                <input id="datatableSearch_" type="search" name="search" class="form-control" placeholder="{{translate('search by ID or restaurant name')}}" aria-label="Search" value="{{$search??''}}" required="" autocomplete="off">
                ...
                <div class="table-responsive datatable-custom">
                    <table
                        ...
                        <tr>
                            ...
                            <th>{{translate('Outdoor Slots')}}</th>
                            <th>{{translate('Indoor Slots')}}</th>
                        ...
                        @foreach($branches as $key=>$branch)
                            <tr>
                                ...
                                <!-- <td>{{$branch['email']}}</td> -->
                                <!--<td>{{$branch->id == 1 ? translate('main_restaurant') : translate('sub_restaurant')}}</td>-->
                                <td>
                                    <div>
                                        <strong><label href="mailto:{{$branch['email']}}" class="mb-0 text-dark bold fz-12"">{{$branch['email']}}</a></strong><br>
                                ...
                                <td>
                                    <div>
                                    <label class="mb-0 text-dark bold fz-12">{{$branch['outdoor_slots']}}</label>
                                    </div>
                                </td>
                                <td>
                                    <div>
                                        <label class="mb-0 text-dark bold fz-12">{{$branch['indoor_slots']}}</label>
                                    </div>
                                </td>
                                        ...
                                        <div class="d-flex justify-content-center gap-3">
                                            ...
                                            @if($branch['id']!=1)
                                                <button type="button" class="btn btn-outline-danger btn-sm delete square-btn"
                                                        onclick="form_alert('branch-{{$branch['id']}}','{{translate('Want to delete this restaurant ?')}}')"><i class="tio-delete"></i></button>
                                            @endif
                                        </div>
